...
## -- Define the class Path which is a sprite 
class Path(pygame.sprite.Sprite): 
    ...
    def Path_setx_val(self, x):
        self.rect.x = x
    
    def Path_sety_val(self, y):
        self.rect.y = y


## -- Define the class Shop which is a sprite 
class Shop(pygame.sprite.Sprite): 
    ...
    def Shop_setx_val(self, x):
        self.rect.x = x
    
    def Shop_sety_val(self, y):
        self.rect.y = y
...
class NPC(pygame.sprite.Sprite): 
    ...
    def NPC_setx_val(self, x):
        self.rect.x = x
    
    def NPC_sety_val(self, y):
        self.rect.y = y
...
class Player(pygame.sprite.Sprite): 
    ...
    def update(self): 
        ...
        # Check collision with the Boss sprite
        ...
        # Check collision with Shop sprite
        if self.rect.colliderect(shop.rect):
            # Adjust player position to prevent overlap
            if self.movex > 0:
                self.rect.right = shop.rect.left
            elif self.movex < 0:
                self.rect.left = shop.rect.right
            if self.movey > 0:
                self.rect.bottom = shop.rect.top
            elif self.movey < 0:
                self.rect.top = shop.rect.bottom

        # Check collision with Path sprite
        if self.rect.colliderect(path.rect):
            # Adjust player position to prevent overlap
            if self.movex > 0:
                self.rect.right = path.rect.left
            elif self.movex < 0:
                self.rect.left = path.rect.right
            if self.movey > 0:
                self.rect.bottom = path.rect.top
            elif self.movey < 0:
                self.rect.top = path.rect.bottom
...
shop_group = pygame.sprite.Group() # Create a seperte group for the shop
buy_group = pygame.sprite.Group() # Create a seperte group for the buy buttons in the shop
path_group = pygame.sprite.Group() # Creates a seperate group for paths
...
Buttonbuy1 = Button(WHITE, 300, 80)
buy_group.add(Buttonbuy1)

Buttonbuy2 = Button(WHITE, 300, 80)
buy_group.add(Buttonbuy2)

Buttonbuy3 = Button(WHITE, 300, 80)
buy_group.add(Buttonbuy3)

#Path
path = Path(GREY, 120, 60)
path_group.add(path)

#Shop
shop = Shop(PURPLE, 100, 100)
shop_group.add(shop)

click_detector = False
click_detector2 = False
click_detector3 = False
click_detector4 = False
click_detector5 = False
click_detector6 = False

shop_detector1 = False
shop_detector2 = False
shop_detector3 = False
...
while not done:
    ...
    if gamestate == 2:
        ...
        if Player.HP <= 0:
            Player.player_setx_val(999)
            Player.player_sety_val(999)
    ...
    if gamestate == 3:
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                # Check if the mouse button was clicked on the NPC sprite
                if event.button == 1 and NPCr.rect.collidepoint(event.pos):
                    click_detector3 = True
                # Check if the mouse button was clicked on the Enemy sprite
                if event.button == 1 and Boss.rect.collidepoint(event.pos):
                    click_detector4 = True
                # Check if the mouse button was clicked on the path sprite
                if event.button == 1 and path.rect.collidepoint(event.pos):
                    click_detector5 = True
                # Check if the mouse button was clicked on the shop sprite
                if event.button == 1 and shop.rect.collidepoint(event.pos):
                    click_detector6 = True
            elif event.type == pygame.KEYDOWN:  # - a key is down
                if event.key == pygame.K_a:  # - if the left key is pressed
                    Player.player_set_movement(-4, 0)  # speed set to -3
                elif event.key == pygame.K_d:  # - if the right key is pressed
        ...
        Area3 = my_fontbig.render('Grasslands', False, (255, 255, 255))
        screen.blit(Area3, (5, 5))

        # Display text if NPC sprite is clicked
        if click_detector3:
            af_txt = my_font.render('Well done on defeating the enemy', False, (255, 255, 255))
            af_txt2 = my_font.render('follow the path ahead or buy from the shop', False, (255, 255, 255))
            screen.blit(af_txt, (240, 20))
            screen.blit(af_txt2, (200, 40))

        # Display text if Enemy sprite is clicked
        if click_detector4:
            gamestate = 5
        
        # Display text if Path sprite is clicked
        if click_detector5:
            Area4 = my_fontbig.render('Grasslands: Boss', False, (255, 255, 255))
            screen.blit(Area4, (5, 5))
            all_sprites_group.draw(screen)

            #sent enemy coords
            Boss.enemy_setx_val(580)
            Boss.enemy_sety_val(100)
            ...
            #set shop coords
            shop.Shop_setx_val(800)
            shop.Shop_sety_val(999)

            #set path coords
            path.Path_setx_val(999)
            path.Path_sety_val(999)

        # Display text if Shop sprite is clicked
        if click_detector6:
            buy_group.draw(screen)
            Area5 = my_fontbig.render('Grasslands: Shop', False, (255, 255, 255))
            screen.blit(Area5, (5, 5))
            ...
            #set player coords
            Player.player_setx_val(360)
            Player.player_sety_val(470)

            #sent enemy coords
            Boss.enemy_setx_val(999)
            Boss.enemy_sety_val(999)
            ...
            #set shop coords
            shop.Shop_setx_val(999)
            shop.Shop_sety_val(999)

            #set sword button pos
            Buttonbuy1.button_setx_val(230)
            Buttonbuy1.button_sety_val(160)

            #set armour button pos
            Buttonbuy2.button_setx_val(230)
            Buttonbuy2.button_sety_val(260)
                    
            #set boots button pos
            Buttonbuy3.button_setx_val(230)
            Buttonbuy3.button_sety_val(360)

            BuyButton1 = my_fontbig.render('Sword', False, (0, 0, 0))
            screen.blit(BuyButton1, (345, 160))
            BuyButtondesc1 = my_fontbig.render('Increases damage by 5', False, (0, 0, 0))
            screen.blit(BuyButtondesc1, (250, 190))

            BuyButton2 = my_fontbig.render('Armour', False, (0, 0, 0))
            screen.blit(BuyButton2, (340, 260))
            BuyButtondesc2 = my_fontbig.render('Increases health by 10', False, (0, 0, 0))
            screen.blit(BuyButtondesc2, (250, 290))

            BuyButton3 = my_fontbig.render('Boots', False, (0, 0, 0))
            screen.blit(BuyButton3, (347, 360))
            BuyButtondesc3 = my_fontbig.render('Guaranteed to flee', False, (0, 0, 0))
            screen.blit(BuyButtondesc3, (270, 390))

            shop_txt = my_font.render('Please select an item to buy by clicking on one of the buttons', False, (255, 255, 255))
            shop2_txt = my_font.render('each item costs 10 credits', False, (255, 255, 255))
            screen.blit(shop_txt, (140, 80))
            screen.blit(shop2_txt, (280, 100))

            if event.type == pygame.MOUSEBUTTONDOWN:
                if event.button == 1 and Buttonbuy1.rect.collidepoint(event.pos):
                    shop_detector1 = True

                if event.button == 1 and Buttonbuy2.rect.collidepoint(event.pos):
                    shop_detector2 = True
                
                if event.button == 1 and Buttonbuy3.rect.collidepoint(event.pos):
                    shop_detector3 = True
    ...
    if gamestate == 4:
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                # Check if the mouse button was clicked on the Boss sprite
                if event.button == 1 and Boss.rect.collidepoint(event.pos):
                    click_detector4 = True
            elif event.type == pygame.KEYDOWN:  # - a key is down
                if event.key == pygame.K_a:  # - if the left key is pressed
        ...
        path_group.draw(screen)
        shop_group.draw(screen)
        Area4 = my_fontbig.render('Grasslands: Boss Fight', False, (255, 255, 255))
        screen.blit(Area4, (5, 5))
    ...
    if gamestate == 5:
        ...
        if click_detectorf:
            ...
            if damage_flag == 0:
                damage = get_damage()
                Boss.enemy_attacked(damage)
                damage_flag = 1
            if Player.Burst == 5:
               damage = get_damage()
               Boss.enemy_attacked(damage * 2)
               Player.Burst = 0
               burst_flag = 1 
               damage_flag = 1 
            # end if
        ...
        #set enemy pos
        Boss.enemy_setx_val(450)
        Boss.enemy_sety_val(175)
        if Boss.HP <= 0:
            Boss.enemy_setx_val(999)
            Boss.enemy_sety_val(999)
        ...
        #set area text
        Area2 = my_fontbig.render('Grasslands: Boss Fight', False, (255, 255, 255))
        screen.blit(Area2, (5, 5))
        #if turnflag == 1:
            #Turn = my_fontbig.render('Your Turn: Choose an action ', False, (255, 255, 255))
            #screen.blit(Turn, (5, 50))
